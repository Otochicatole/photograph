        ...
        <header  ref={headerRef}/>
            ...
            <div className="mt-[140px] w-full  flex flex-col justify-center items-center">
                <section className="flex flex-col w-full p-4 xl:w-[80%] items-center text-center">
                    ...
                    <p>Permettimi di introdurmi tra le storie che ho fotografato in questi dieci meravigliosi anni: matrimoni,  <br />
                        bambini e feste che mi hanno permesso di ritrarre ogni emozione più bella:</p>
                    <p className=" italic text-[#545454] mb-20 font-[LigthItalic] text-center p-3">l’amore e la felicità.</p>
                    {/* ------- */}
                    {Object.entries(groupedData).map(([sectionType, items]) => (
                        <div key={sectionType} className="mb-8 ">
                            <div className="flex flex-col w-full items-center justify-center">
                            <h2 className="text-[32px] font-[Ovtreasure] text-[#545454] border border-[#545454] px-6 rounded-4xl mb-16">{sectionType}</h2>
                            </div>
                            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-[1px]">
                                {items.map((item, index) => (
                                    <div key={index} className="p-1">
                                        <Link to={`/gallery/${encodeName(item.name)}`} className="flex p-4 rounded-[25px] hover:bg-[#ffe3de] cursor-pointer transition-all ease-in-out duration-300">
                                        {item.images && (
